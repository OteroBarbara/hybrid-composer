...
function hc_get_post_type_item_box($item, &$Y_NOW) {
    $box_type = $Y_NOW["box"];
    $hidden = hc_true($Y_NOW["hidden_content"]);
    $animation = $Y_NOW["box_animation"];
    $extra_1 = hc_true(hc_get_now($Y_NOW,"extra_1",false));
    $extra_2 = hc_true(hc_get_now($Y_NOW,"extra_2",false));
    ...
    if ($Y_NOW["button_text"] != "") {
        ...
        $button = '<a class="' . $button . '" href="' . esc_url($item["link"]) . '">' . ((hc_get_now($Y_NOW,"button_animation")) ? '<i class="fa fa-long-arrow-right"></i> ':'') . hc_json($Y_NOW["button_text"]) . '</a>';
    }
    ...
    $title = hc_json($item["title"]);
    $excerpt = hc_json($item["excerpt"]);
    ...
    if (hc_true($Y_NOW["boxed_inverse"])) $css .= " boxed-inverse";
...
<div class="advs-box advs-box-side <?php echo $css; ?>" data-anima="fade-left" data-trigger="hover">
    <div class="row">
        ...
        <div class="<?php if ($img != "") echo "col-md-8"; else echo "col-md-12" ?>">
            ...
            <hr class="anima" />
            <?php if ($extra_1) hc_echo($item["extra_1"],' <p class="box-extra-value extra-1">','</p>'); if ($extra_2) hc_echo($item["extra_2"],' <p class="box-extra-value extra-2">','</p>'); ?>
...
<?php }
    if ($box_type == "top_icon") {
        $style = "";
        if ($item["icon"]["icon"] == "") {
            $style = 'style="margin-top: 0 !important;"';
            $css .= ' no-icon';
        }
?>
...
<div class="advs-box advs-box-top-icon-img <?php echo $css ?>">
    <?php hc_echo(hc_get_now($item, array("icon", "icon")), "<i class='fa ", "'></i>"); ?>
...
<div class="advs-box advs-box-multiple <?php echo $css ?>" data-anima="scale-up" data-trigger="hover">
    ...
    <div class="circle anima">
        <?php if ($extra_1 && strlen($item["extra_1"]) > 0) echo $item["extra_1"];
    ...
    <div class="advs-box-content">
        ...
        <?php
        if ($extra_1) hc_echo($item["extra_1"],' <p class="box-extra-value extra-1">','</p>');
...
<div class="advs-box advs-box-top-icon-img niche-box-post <?php echo $css ?>">
    ...
    <div class="advs-box-content">
        ...
        <?php hc_get_post_tags($item,$Y_NOW["post_type_slug"],true,true,true,true,true) ?>
        <?php if ($extra_1) hc_echo($item["extra_1"],' <p class="box-extra-value extra-1">','</p>'); if ($extra_2) hc_echo($item["extra_2"],' <p class="box-extra-value extra-2">','</p>'); ?>
        <?php if (($content == "" || $content == "description") && $excerpt != "") echo '<p>' . $excerpt . '</p>'; ?>
        <?php echo $button ?>
    </div>
...
<div class="img-box adv-img adv-img-half-content <?php hc_echo($animation," img-",""); echo $css; ?>" <?php if ($hidden) echo 'data-anima="fade-bottom" data-trigger="hover" data-anima-out="hide"' ?>>
    ...
    <div class="caption <?php if ($hidden) echo "anima" ?>">
        <h2>
            <a href="<?php echo esc_url($item["link"]); ?>" class="<?php echo $title_size; ?>">
                <?php echo esc_html($title); ?>
            </a>
...
<div class="img-box adv-img adv-img-side-content i-top-right <?php hc_echo($animation,"img-",""); echo $css; ?>" <?php if ($hidden) echo 'data-anima="fade-left" data-trigger="hover" data-anima-out="hide"' ?>>
...
<div class="img-box adv-img adv-img-full-content <?php echo $css ?>" <?php if ($hidden) echo 'data-anima="' . $animation . '" data-trigger="hover" data-anima-out="hide"' ?>>
    <div class="img-box">
        <img alt="" src="<?php echo hc_get_img($item["image"],"ultra-large") ?>" />
    </div>
...
<div class="img-box adv-img adv-img-button-content <?php hc_echo($animation,"img-",""); echo $css; ?>" data-anima="fade-left" data-trigger="hover" <?php if ($hidden) echo 'data-anima-out="hide"' ?>>
    <a href="#">
        <img alt="" src="<?php echo hc_get_img($item["image"],"ultra-large") ?>" />
    </a>
    <div class="caption <?php if ($hidden) echo "anima anima-fade-top" ?>">
        <h2 class="<?php echo $title_size; ?>">
            <?php echo esc_html($title); ?>
        </h2>
        <?php hc_echo_box_text($content, $item["subtitle"],  $excerpt, "sub", "sub", $item["extra_1"]) ?>
...
<a href="<?php echo esc_url($item["link"]); ?>" class="img-box adv-circle circle adv-circle-center <?php if (!$hidden) echo 'show' ?>" data-anima="<?php if ($animation == "") echo "fade-in"; else echo $animation; echo $css; ?>" data-trigger="hover" <?php if ($hidden) echo 'data-anima-out="hide"' ?>>
...
<a href="<?php echo esc_url($item["link"]); ?>" class="img-box adv-circle circle adv-circle-center-2 <?php if (!$hidden) echo 'show' ?>" data-anima="<?php if ($animation == "") echo "fade-in"; else echo $animation;echo $css; ?>" data-trigger="hover" <?php if ($hidden) echo 'data-anima-out="hide"' ?>>
...
<a href="<?php echo esc_url($item["link"]); ?>" class="img-box adv-circle circle <?php hc_echo($animation,"img-"," "); if (!$hidden) echo 'show'; if ($box_type == "image_circle_bottom") echo " adv-circle-bottom"; else echo " adv-circle-half"; echo $css; ?>" data-anima="fade-in" data-trigger="hover" <?php if ($hidden) echo 'data-anima-out="hide"' ?>>
...
<a href="<?php echo esc_url($item["link"]); ?>" class="img-box adv-circle circle adv-circle-bg  <?php if (!$hidden) echo 'show'; echo $css; ?>" data-anima="<?php if ($animation == "") echo "fade-in"; else echo $animation; ?>" data-trigger="hover" <?php if ($hidden) echo 'data-anima-out="hide"' ?>>
    <?php hc_echo($item["icon"]["icon"],'<i class="',' main-icon anima"></i>'); ?>
    <img alt="" src="<?php echo hc_get_img($item["image"],"ultra-large") ?>" />
    <div class="caption anima">
        <div class="inner">
...
<div class="grid-list <?php echo hc_get_component_classes($Y_NOW,$EXTRA); if($Y_NOW["column"] == "col-md-12") echo " one-row-list"; ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <div class="pagination-inner<?php if (hc_true($Y_NOW["pag_centered"])) echo " list-nav" ?>">
        <ul class="<?php echo esc_attr($Y_NOW["button_size"]); ?> hide-first-last pagination-grid" data-page-items="<?php if (strlen($Y_NOW["pag_items"]) > 0 ) echo esc_attr($Y_NOW["pag_items"]); else echo "10"; ?>" <?php hc_echo(esc_attr($Y_NOW["pag_lm_animation"]),"data-pagination-anima='","'"); ?>
            data-options="<?php if (hc_true($Y_NOW["pag_scroll_top"])) echo "scrollTop:true,"; hc_echo(esc_attr($Y_NOW["pag_button_prev"]),"prev:<i class='fa fa-angle-left'></i> ",","); hc_echo(esc_attr($Y_NOW["pag_button_next"]),"next:","<i class='fa fa-angle-right'></i>"); ?>"></ul>
    </div>
    <?php  }
    ...
    <?php }
        if ($Y_NOW["pagination_type"] == "pagination_wp") {
            ...
            if (hc_true($Y_NOW["pag_centered"])) echo "</div>";
        }
    }
    ?>
</div>
...
function hc_include_hc_pt_masonry_list(&$Y_NOW,$EXTRA) {
    ...
    if (hc_true($Y_NOW["auto_masonry"])) $classes .= " maso-layout";
...
<div class="maso-list <?php echo $classes ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <div class="navbar navbar-inner">
        ...
        <div class="collapse navbar-collapse <?php if ($Y_NOW["menu_position"] == "nav-center") echo "text-center";
                                                   if ($Y_NOW["menu_position"] == "nav-right") echo "text-right" ?>">
            <ul class="nav navbar-nav over inner maso-filters <?php echo esc_attr($Y_NOW["menu_style"] . " " . $Y_NOW["menu_position"]); ?>">
                <li class="current-active active">
                    <a class="maso-filter-auto" data-filter="maso-item"><?php esc_html_e("All","hc"); ?></a>
                </li>
                <?php
    ...
    <?php if ($Y_NOW["pag_centered"] == "true") echo "</div>";
        ...
        if ($Y_NOW["pagination_type"] == "load_more") {  ?>
    <div class="list-nav">
        <a class="btn-border btn btn-sm load-more-maso" <?php hc_echo(esc_attr($Y_NOW["pag_lm_animation"]),"data-pagination-anima='","'"); ?>
            data-page-items="<?php if (strlen($Y_NOW["pag_items"]) > 0 ) echo esc_attr($Y_NOW["pag_items"]); else echo "3"; ?>"
            data-options="<?php if (strlen($Y_NOW["lm_lazy"]) > 0 ) echo "lazyLoad:true"; ?>">
            <?php echo esc_attr($Y_NOW["lm_button_text"]); ?>
            <i class="fa fa-arrow-down"></i>
...
<div class="flexslider <?php echo esc_attr($Y_NOW["slider_type" ]). ' ' . $Y_NOW["css_classes"] . ' ' . $Y_NOW["custom_css_classes"]; if (hc_true($Y_NOW["nav_inner"])) echo " nav-inner"; if (hc_true($Y_NOW["outer_arrows"])) echo " outer-navs"; if (hc_true($Y_NOW["visible_arrows"])) echo " visible-dir-nav"; ?>"
