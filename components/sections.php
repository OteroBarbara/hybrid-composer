...
//gutenberg all pagas due to hc_json()
...
function hc_include_hc_section(&$Y_NOW,$EXTRA) {
    ...
    if (hc_true(hc_get_now($SECTION_SETTINGS,"parallax"))) $section_classes .= " parallax-window";
    if (hc_true(hc_get_now($SECTION_SETTINGS,"full_screen"))) $section_classes .= " full-screen-title";
    if (hc_true($Y_NOW['box_middle'])) {
        $section_classes .= " box-middle-container";
        array_push($SUB_EXTRA, "box-middle");
    }
    if (hc_true($Y_NOW["timeline_animation"])) array_push($SUB_EXTRA, "timeline");
    if (!in_array("fullpage", $EXTRA)) {
        if (hc_true($Y_NOW['vertical_row'])) array_push($SUB_EXTRA, "vertical-row");
...
<div id="<?php echo esc_attr($Y_NOW['id']); ?>" class="section-item section-bg-image <?php echo $section_classes ?>"
    ...
    ?>
    style="<?php if (!hc_true($SECTION_SETTINGS['parallax'])) echo 'background-image: url(' . hc_get_img($SECTION_SETTINGS['image'],"full") . '); ';  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
...
<div data-id="<?php echo esc_attr($Y_NOW['id']); ?>" class="<?php if (in_array("fullpage_horizontal",$EXTRA)) echo "slide "; else echo "section "; echo $section_classes; ?> overlay-container" <?php hc_get_scroll_animation($Y_NOW["animation"],$Y_NOW["animation_time"],$Y_NOW["timeline_animation"],$Y_NOW["timeline_delay"],$Y_NOW["timeline_order"],(in_array("fullpage", $EXTRA) ? "fullpage":"")) ?> style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
        ...
        if (hc_true($Y_NOW['vertical_row'])) array_push($EXTRA, "vertical-row");
...
function hc_include_hc_section_two_blocks(&$Y_NOW,$EXTRA) {
    ...
    if (hc_true($Y_NOW["inverse"])) $css .= "blocks-right";
    ...
    if ($fp) $css .= " section";
    if (hc_true(hc_get_now($Y_NOW,"boxed")))  $css .= " two-blocks-container";
?>
<div <?php echo (($fp) ? 'data-id="':'id="') . $Y_NOW['id'] ?> " class="section-item section-two-blocks <?php echo $css ?>"
    <?php hc_echo(esc_attr($Y_NOW["custom_css_styles"]),"style='","'");
          ...
          if (hc_true($Y_NOW["parallax"])) echo ' data-parallax="scroll" data-image-src="' . hc_get_img($Y_NOW["image"],"hd") .'"';  hc_get_scroll_animation($Y_NOW["animation"],hc_get_now($Y_NOW,"animation_time"),hc_get_now($Y_NOW,"timeline_animation"),hc_get_now($Y_NOW,"timeline_delay"),hc_get_now($Y_NOW,"timeline_order"),(in_array("fullpage", $EXTRA) ? "fullpage":"")) ?>>
