...
$title_arr = hc_get_the_title();
if (hc_true(hc_get_now($NOW_SUB,"white"))) $white = " white ";
if ($NOW_SUB["component"] == "hc_title_image") {
?>
<div class="header-title overlay-container <?php echo $white . esc_attr($NOW_SUB['ken_burn']); if (hc_true($NOW_SUB['full_screen'])) echo " full-screen-title";?>"
    <?php  $tmp = explode("|",$NOW_SUB['image']);
           if (hc_true($NOW_SUB['parallax']) && count($tmp) > 1) {
    ...
    <div class="overlay-content overlaybox">
        <div class="container">
            <div class="title-base <?php echo hc_get_now($NOW_SUB,"position") ?>">
                <hr class="anima" />
                <h1>
                    <?php echo esc_attr($title_arr[0]) //gutenberg ?>
                </h1>
                <p>
                    <?php echo hc_json($Y_NOW['subtitle']); ?>
                </p>
                <?php if (hc_true(hc_get_now($NOW_SUB,"breadcrumbs"))) echo hc_get_breadcrumb(); ?>
            </div>
        </div>
    </div>
</div>
...
if ($NOW_SUB["component"] == "hc_title_slider") {
    $css = $white;
    if (hc_true($NOW_SUB['full_screen'])) $css .= "full-screen-title ";
    if (hc_true($NOW_SUB['parallax'])) $css .= "header-parallax";
?>
<div class="header-slider <?php echo $css ?>" data-sub-height="<?php echo esc_attr($NOW_SUB['full_screen_height']) ?>">
    <?php if (hc_true($NOW_SUB['parallax'])) echo '<div class="layer-parallax">'; ?>
    ...
    <?php if (hc_true($NOW_SUB['parallax'])) echo '</div>'; ?>
...
<div class="header-video <?php echo $css ?>" data-sub-height="<?php echo esc_attr($NOW_SUB['full_screen_height']) ?>">
    ...
    <div class="videobox <?php if (hc_true($NOW_SUB['parallax'])) echo " layer-parallax" ?>">
...
<div class="header-base <?php echo $css ?>" <?php hc_echo(hc_get_img($img),'style="background-image: url(',')"') ?>>
    <div class="container">
        <div class="row">
            <div class="<?php if (hc_true(hc_get_now($NOW_SUB,"breadcrumbs"))) echo "col-md-8"; else echo "col-md-12"; ?>">
            ...
            <?php if(!isset($NOW_SUB["breadcrumbs"]) || hc_get_now($NOW_SUB,"breadcrumbs",0)) echo '<div class="col-md-4">' . hc_get_breadcrumb() . '</div>'; ?>
        </div>
    </div>
</div>
<?php } ?>